...
const { serverToday, istToday, addDays } = require('../utils/dates');
...
router.post('/daily-tasks', requireAuth, asyncRoute(async (req, res) => {
  ...
  // Only today or yesterday may be filled, judged in IST like every other date
  // in the app. This used to take local midnight and then call toISOString(),
  // which converts to UTC — in IST that lands on the previous day, so the form
  // rejected the very date it had just offered as "Today".
